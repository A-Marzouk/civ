...
class PayPalDirectPayouts extends Controller
{
    private $apiContext;
    private $client_id;
    private $secret;
    ...
    public function directPayPalPayments(Request $request)
    {
    ...
    protected function makeOneTimePayment($request)
    {
        ...
        $payee = new Payee();
        $payeeEmail = $this->getUserPaymentEmail($request);
        if(!$payeeEmail){
            return ['error' => 'No Payment Method provided'];
        }
    ...
    protected function getUserPaymentEmail($request){
        $user = User::where('email', $request->freelancer['email'])->first();
        return $user->paypalPaymentEmail();
    }
